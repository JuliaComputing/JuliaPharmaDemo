...
# ╔═╡ 311a9f52-7210-11ee-2888-cffad2646c5a
using ModelingToolkit, DifferentialEquations, OrdinaryDiffEq, DataFrames, CSV, Plots, StatsPlots

# ╔═╡ c232b215-9cce-46ea-8c92-935f4f4b4eec
md"""
# Example workflow

Viral B. Shan and Elisabeth Roesch (JuliaHub, Inc.)
"""

# ╔═╡ c341495b-e38a-4b39-a986-0bbe8c8624bf
md"""
## Packages in Julia
"""

# ╔═╡ 79646f66-7549-4d8c-9e80-959e7015e4d7
md"""
The Julia ecosystem contains over 10,000 packages that are registered in the General registry and extend core Julia functionality.
![](https://media.springernature.com/full/springer-static/image/art%3A10.1038%2Fs41592-023-01832-z/MediaObjects/41592_2023_1832_Fig2_HTML.png?as=webp)
"""

# ╔═╡ bf73fa11-1995-410a-8d77-83f14c6a6570
md"""
Figure from: Roesch, E., et al. Julia for biologists. Nat Methods (2023).
"""

# ╔═╡ 4c618cd1-6930-4027-80ca-e5fdb09a2382
# A model defined via ModelingToolkit
begin 
	@variables begin
	    t 
	    y1(t) = 1.0  
	...
	D = Differential(t)
	eqs = [
	    D(y1) ~ (-k1*y1 + k5*y2 + k3*y3 + k7),
	    D(y2) ~ (k1*y1 + k8*y2),
	    D(y3) ~ (k10*y3 + k5*y4 + k9*y5),
	    D(y4) ~ (k3*y2 + k1*y3 + k11*y4),
	    D(y5) ~ (k12*y5 + k5*y6 + k5*y7),
...
# ╔═╡ 4cceb613-f01c-4f8b-9764-8458c1e0f713
md"""
## Reading, writing CSV files and DataFrames in Julia
Helpful pkgs: [CSV.jl](https://csv.juliadata.org/stable/), [DataFrames.jl](https://dataframes.juliadata.org/stable/).
"""

# ╔═╡ c5f41dcf-4ab4-4f57-80e2-0752f619b3dc
# Clinical condition 1
begin 
	ode_prob_clin_1 = ODEProblem(QSPmodel, [], (0.0, 100000.0), [k6 => 20.0])
	ss_prob_clin_1 = SteadyStateProblem(ode_prob_clin_1)
	ss_sol_clin_1 = solve(ss_prob_clin_1, DynamicSS(QNDF()))
	df_clin_1 = DataFrame(["y1(t)", "y2(t)", "y3(t)", "y4(t)", "y5(t)", "y6(t)", "y7(t)", "y8(t)"] .=> ss_sol_clin_1)
	CSV.write("df_clin_1.csv", df_clin_1)
end;

# ╔═╡ a9345233-37a8-4949-b382-9f80c48628cd
md"""
##### Data 1: Steady state data - Normalization/Adaption
"""

# ╔═╡ 574b18e8-7f23-475c-9d08-430bbfe1b356
# Read steady state data of clinical condition 1 from CSVs
df_clin_cond_1 = CSV.read("df_clin_1.csv", DataFrame)

# ╔═╡ 58369e23-43eb-4937-88b2-f3729dbefef2
# Clinical condition 2
begin	
	ode_prob_clin_2 = ODEProblem(QSPmodel, ss_sol_clin_1.u, (0.0, 20.0), [k3 => 15.0, k4 => 0.6, k6 => 2.0])
	ode_sol_2 = solve(ode_prob_clin_2, saveat = .1)
	df_clin_2 = DataFrame(ode_sol_2)
	CSV.write("df_clin_2.csv", df_clin_2)
end;

# ╔═╡ de204c13-6ce5-48b0-9651-80045dde15fb
md"""
##### Data 2: Time series data - Long term effect
"""

# ╔═╡ 8329aa7d-9b3d-4474-bfc9-b1f84fefdd6e
# Read time series data of clinical condition 2 from CSVs
df_clin_cond_2 = CSV.read("df_clin_2.csv", DataFrame)

# ╔═╡ 544e5084-0568-4ea1-9050-c0355ac52ee2
md"""
##### Analyzing and visualizing data
"""

# ╔═╡ 56a214c4-dd48-43e7-8cc4-788178ff1566
md"""
###### Comparing columns
We use the dotted operators (`./` and `.-` and `.^`). Multiple vectorized operations are "fused" into a single loop!
"""

# ╔═╡ 5c8b7701-1ed5-4ece-8bf9-7d9653aa1c11
# Visualize data 1: Ratio of states y3 and y4
begin
	x = df_clin_cond_2."timestamp"
	y = ((df_clin_cond_2."y3(t)"./df_clin_cond_2."y4(t)").-df_clin_cond_2."y5(t)").^2
	p1_all = plot(x, y, xlabel = "time", ylabel = "Normalized ratio", marker = :o, color = "green", label = "All days")
	p2_all = violin([""], y[3:7:end], side = :right, color = "green", label = "")
	g_all = grid(1, 2, widths = [0.8, 0.2])
	plot(p1_all, p2_all, layout = g_all)
end

# ╔═╡ 76379f51-0067-4b3a-95ce-fe19359b0955
md"""
The speed of Julia.
![](https://media.springernature.com/full/springer-static/image/art%3A10.1038%2Fs41592-023-01832-z/MediaObjects/41592_2023_1832_Fig3_HTML.png?as=webp)
"""

# ╔═╡ 09f2ed5c-cbdb-498a-89ea-33055ae6e3a8
md"""
###### Adding interactivity
Adding `html` element to visualize only specific weekdays.
"""

# ╔═╡ 6cfb40b5-65a6-4302-9403-3ed2ac166331
md"""
> Which day of the week would you like to look at?
"""

# ╔═╡ 445f404b-e7fb-464e-82e5-00f4678619d6
@bind day html"<input type=range min=1 max=7>"

# ╔═╡ 1e526aa3-b32b-48e3-90a8-3e1d65db7cf3
begin
	name =""
	if day==1
		name = "Monday"
...
# ╔═╡ 0f08fb12-3094-4c2e-a5d6-52219c0310c5
# Visualize data: Every Wednesday, ratio of states y3 and y4
begin
	p1 = plot(x[day:7:end], y[day:7:end], xlabel = "time", marker = :o, color = "green", ylabel = "Normalized ratio", label = "$name")
	p2 = violin([""], y[3:7:end], side = :right, color = "green", label = "")
	g = grid(1, 2, widths = [0.8, 0.2])
	plot(p1, p2, layout = g)
end
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "49386f7653dd127f5cb293aa6f83cace2fb308cb"
...
git-tree-sha1 = "46ad2af2ffe3f0db13c00269d5fdecf66207b3c1"
