...
# ╔═╡ d7a45a2b-5de3-4849-83f1-903eaeabe599
begin 
	using Downloads: download
	using SBMLToolkit, Graphs, SimpleWeightedGraphs, StatsPlots, InformationMeasures, GraphPlot, DataFrames, PumasQSP, ModelingToolkit, Plots, Images, DifferentialEquations
end

# ╔═╡ 6e1e3a8a-e514-4857-a713-62d8df932ff6
md"""
...
Material: `bit.ly/Julia_Pharm`


"""

# ╔═╡ 4a569c2b-0f52-457a-b259-f97c88bf6509
begin
img_path = "qr.png"
img_data = load(img_path)
plot(img_data, axis = :off, grid = :off)
end

# ╔═╡ 19b35f87-5ac9-4799-a00c-97bd9e4588be
md"""
## Why Julia?
Julia solves the **two-language problem**!
![](https://media.springernature.com/full/springer-static/image/art%3A10.1038%2Fs41592-023-01832-z/MediaObjects/41592_2023_1832_Fig1_HTML.png?as=webp)
"""

# ╔═╡ 0fd6608d-2304-4fa7-9181-c78f33c353e6
md"""
Real world impact in the pharmaceutical industries:
...
# ╔═╡ 9b460cbe-e179-4cfa-b74e-0a5391e43d3f
md"""
## What does Julia look like?

Here comes a little Modeling and Simulation example: 
"""

# ╔═╡ 513ca4b7-aafa-4354-bc52-28c462d4a1bd
md"""
1. First we prepare the environment.
"""

# ╔═╡ 041f0168-2046-46a5-a087-ac9523b36652
md"""
2. We download an SBML model and import it into ModelingToolkit, the software package we use as our modeling framework. We initialise the system, interprete it as an ODE, and simulate.
"""

# ╔═╡ 7a6a75fa-8564-4530-bc56-6061a3dc4869
begin
	download("https://ftp.ebi.ac.uk/pub/databases/biomodels/repository/aaj/MODEL2107190002/5/Bakshi2020%20truncated%20minimal%20model.xml", "_model.xml")
...
# ╔═╡ 2c2319a6-4dea-4605-a5bf-005a0c172b68
md"""
3. We set the number of cells that are simulated.
"""

# ╔═╡ 28b10b6f-44a4-4329-acdc-988a3ea9ca6f
@bind n_cells html"<input type='range' min = 5 max = 100>" 

# ╔═╡ 921acee7-d416-4621-9cc6-355047757de1
print(n_cells)

# ╔═╡ 7c39a977-ed05-454e-827c-8702f8128b06
begin
	@unpack k1 = odesys
	...
	ts = ens_sol[1].t
	t_length = length(ts)
	plot(ens_sol, grid = "off")
end

# ╔═╡ 7454e0ce-50aa-4b32-a1c4-784e5dec4332
md"""
4. We can easily visualise results, too.
"""

# ╔═╡ 7a2bf1fa-b254-45e3-bc53-5582e01e43a8
begin 
	#cells/trajectories and genes/states
	function diff_(a,b)
	    return sum((a.-b).^2)
	end
	v = Vector{Matrix{Float64}}(undef, t_length)
	for (x, temp_t) in enumerate(ts)
	    ...
	    for (i, s1) in enumerate(sts)    
	        for (j, s2) in enumerate(sts)
	            a[i,j] = get_mutual_information(ens_sol(temp_t, idxs = s1), 							ens_sol(temp_t, idxs = s2))*100
	        end
	    end
	    v[x] = a
	end
...
# ╔═╡ 39337f46-96b1-4658-a8b5-9fa1053a8c63
md"""
Select the timepoint you want to visualise:
"""

# ╔═╡ 767d1139-fc6a-4517-84de-a0128eff87c7
@bind select_timepoint html"<input type='range' min = 1 max = 11>" 

# ╔═╡ 13c3c245-51d5-4bf6-9a53-6cb431fa6ed0
print(select_timepoint)

# ╔═╡ 7ced942b-b72e-433f-9c8b-e662c94f2459
md"""
Select the cut off weight:
"""

# ╔═╡ ac6ec1d2-beae-48df-a3bb-6a830c086a4f
@bind cut_off_weight html"<input type='range' min = 0 max = 11>" 

# ╔═╡ 572490fe-b6cd-4dcd-874d-c5df8c0e80b6
print(cut_off_weight)

# ╔═╡ bc363acf-9d8d-4d7c-bb5e-7142b6161dcd
begin
	n_states = length(sts)
	...
	for i in 1:n_states
	    for j in 1:n_states
	        temp_weight  = v[select_timepoint][i,j]
...
# ╔═╡ 42c57de3-a44a-4f50-9844-d32c12129db6
md"""
## More details: Speed
...
# ╔═╡ 6f8865a6-7185-46d8-bb31-9564a60115fc
md"""
## More details: Abstraction
![](https://media.springernature.com/full/springer-static/image/art%3A10.1038%2Fs41592-023-01832-z/MediaObjects/41592_2023_1832_Fig5_HTML.png?as=webp)
"""

# ╔═╡ f9a67cc4-ea18-4d43-acc5-2e5519c82e4b
md"""
## More details: Metaprogramming 
![](https://media.springernature.com/full/springer-static/image/art%3A10.1038%2Fs41592-023-01832-z/MediaObjects/41592_2023_1832_Fig6_HTML.png?as=webp)
"""

# ╔═╡ f628a836-7f91-4f9f-9627-c7a7fe6f2eb1
md"""
## Further Resources
- [User talk: Julia and SciML for QSP Modeling at Sanofi | Katy Norman | JuliaCon 2023](https://www.youtube.com/watch?v=U4dUkbj_ibg)
- [User talk: QSP Model for Pulmonary Arterial Hypertension | Andrew Stine | JuliaCon 2023] (https://www.youtube.com/watch?v=egQHkE3h-0M)
- [Roesch, E., et al. Julia for biologists. Nature Methods, 2023.](https://www.nature.com/articles/s41592-023-01832-z)
"""
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "2c78ef4c540fc6b541db1baec9063c9028dc9e11"
